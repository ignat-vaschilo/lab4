...
int main() {
    ...
    cout << "------------------------------------------------------------" << endl;
    cout << "Введите нужный предмет: ";
    string need_subject;
    cin.get();
    getline(cin, need_subject);
    map<int, int> marksInGroups = {
        {11, 0},
        {12, 0},
        {13, 0},
    };
    for (int i = 0; i < students_vec.size(); i++) {
        for (int j = 0; j < cnt_subjects; j++) {
            //cout << "mi tut " << need_subject << " " << endl;
            if (students_vec[i].subjects[j].name == need_subject) {
                
                if (students_vec[i].subjects[j].mark == "Неудовлетворительно") {
                    marksInGroups[students_vec[i].numOfGroup] += 1;
                }
            }
        }
    }
    for (int i = 11; i < 11+cnt_groups; i++) {
        cout << "В группе №" << i << " оценку Неудовлетворительно по данному предмету имеет " << marksInGroups[i] << " человек" << endl;
    }
